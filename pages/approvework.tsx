                ...
                {newData &&
                  ...
                  search(newData)
                    ...
                    .map((data) => (
                      ...
                      <tr key={data.id} style={{ "backgroundColor" : (data.canSeeWork ? "#00FF0040" : "#FF00FF40")}}>
                        
                        <td>{data.work_title}</td>
                        <td>{data.work_desc}</td>
                        <td>{data.stake_amount / CW20_DECIMAL}</td>
                        <td style={{color:(data.isOverStaked ? "blue" : "red")}}>
                          {
                            (data.canSeeWork ? data.cw20_balance[0].amount / CW20_DECIMAL : "")
                          }
                        </td>
                        <td>{data.canSeeWork ? "" : data.mystakeamount / CW20_DECIMAL}</td>
                        <td>
                          {data.canSeeWork ?
                            <button
                        ...
                        <td>
                          ...
                          {
                            (
                              data.canSeeWork || data.isOverStaked && data.isExpired && data.isIStaked
                            ) ?
                        ...
                        <td>
                          <button
                            ...
                            style={{
                              "backgroundColor": (
                                data.canSeeWork ? "var(--bs-gray)" : (data.isExpired && data.isOverStaked) ? "var(--bs-pink)" : ""
                              ) 
                            }}
                            disabled={data.canSeeWork || data.isExpired && data.isOverStaked}
                            onClick={(e) => handleStakeOpen(data)}
                            >
                                <i className= 'bx bxs-like bx-lg'></i> 
                                {
                                  data.canSeeWork? "Staked" : ((data.isExpired && data.isOverStaked) ? "Started" : "Stake")
                                }
                          </button>
                        </td>
                      </tr>
